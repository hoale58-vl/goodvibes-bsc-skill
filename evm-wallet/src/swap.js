...
/**
 * Swap Script - Swap tokens via Odos aggregator (BSC only)
 * Usage: 
 *   node src/swap.js <chain> <fromToken> <toToken> <amount> [--slippage <percent>] [--yes] [--quote-only]
 *   node src/swap.js bsc eth 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d 0.01
 */
...
function showHelp() {
  console.log(`
EVM Token Swap (Odos Aggregator - BSC Only)

Usage: node src/swap.js [options] <chain> <fromToken> <toToken> <amount>

Arguments:
  chain          Chain name (only 'bsc' is supported)
  fromToken      Token to sell: 'eth'/'bnb' for native, or contract address
  toToken        Token to buy: 'eth'/'bnb' for native, or contract address
...
Examples:
  node src/swap.js bsc bnb 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d 0.01   # Swap 0.01 BNB → USDC on BSC
  node src/swap.js bsc 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d bnb 100    # Swap 100 USDC → BNB on BSC
  node src/swap.js bsc bnb 0x8AC76a51c... 0.01 --slippage 1 --yes            # 1% slippage, skip confirm
  node src/swap.js bsc bnb 0x8AC76a51c... 0.01 --quote-only                  # Just get quote
`);
}
...
function resolveTokenAddress(token) {
  ...
  if (lower === 'eth' || lower === 'bnb' || lower === 'native' || lower === 'pol') {
    return NATIVE_TOKEN;
  }
  if (!isAddress(token)) {
...
async function main() {
  try {
    ...
    // Odos only supports BSC (Chain ID 56) in this context
    if (chainName.toLowerCase() !== 'bsc') {
      exitWithError('Odos swap is currently only supported on BSC (Binance Smart Chain).');
    }
